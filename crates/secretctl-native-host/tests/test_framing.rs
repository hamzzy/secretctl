...
fn test_framing_conversion_le_to_be() {
    ...
    let json_payload = b"{\"jsonrpc\":\"2.0\",\"id\":\"1\",\"method\":\"executor.prepare\"}".to_vec();

    // 1. Chrome encodes as 32-bit Little Endian
    let mut chrome_buf = BytesMut::new();
    chrome_codec.encode(json_payload.clone(), &mut chrome_buf).unwrap();
    ...
    broker_codec.encode(decoded_from_chrome, &mut broker_buf).unwrap();
