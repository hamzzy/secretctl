...
fn test_cdp_side_channel_denials() {
    ...
    assert!(
        filter
            .validate_cdp_command("Network.getCookies", None)
            .is_err()
    );
    assert!(
        filter
    ...
    assert!(
        filter
            .validate_cdp_command("DOMStorage.getDOMStorageItems", None)
            .is_err()
    );
    assert!(
        filter
            .validate_cdp_command("IndexedDB.requestData", None)
            .is_err()
    );
    ...
    assert!(
        filter
            .validate_cdp_command("Input.dispatchMouseEvent", None)
            .is_ok()
    );
}
...
fn test_sensitive_window_isolation() {
    ...
    assert!(
        filter
            .validate_cdp_command("Page.startScreencast", Some(tab_id))
            .is_err()
    );
    assert!(
        filter
...
fn test_header_and_cookie_redaction() {
    ...
    let cookie_header = payload
        .pointer("/params/request/headers/Cookie")
        .unwrap()
        .as_str()
        .unwrap();
    let ua_header = payload
        .pointer("/params/request/headers/User-Agent")
        .unwrap()
        .as_str()
        .unwrap();
